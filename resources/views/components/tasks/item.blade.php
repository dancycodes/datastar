<div id="task-{{ $task->id }}" class="bg-gray-200 p-2 shadow rounded overflow-hidden">
    <div class="flex justify-between items-center ">
        <div class="flex gap-x-2 items-center">
            <input data-on-change="{{ dspatch(route('task.toggle-complete', ['task' => $task->id])) }}" type="checkbox" class="form-checkbox h-4 w-4 text-blue-600" @if($task->is_completed) checked @endif />
        ...
        <div class="flex gap-x-2 items-center">
            <button 
                title="edit"
                data-on-click="@get('{{ route('task.get-form', ['task' => $task->id]) }}')"
            >
            ...
            <button 
                title="delete"
                data-on-click="{{ dsdelete(route('task.destroy', ['task' => $task->id])) }}"
            >
